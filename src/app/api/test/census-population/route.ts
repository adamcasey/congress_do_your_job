...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return jsonError(`Census API returned ${response.status}: ${errorText}`, response.status, {
        state,
    ...
    return jsonError("Failed to fetch Census data", 500, error instanceof Error ? error.message : "Unknown error");
  }
}
