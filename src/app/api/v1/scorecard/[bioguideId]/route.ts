 ...
 */
export async function GET(request: NextRequest, { params }: { params: Promise<{ bioguideId: string }> }) {
  try {
    const { bioguideId } = await params;

    if (!bioguideId || !BIOGUIDE_ID_PATTERN.test(bioguideId)) {
      return jsonError("Invalid bioguideId. Expected format: one uppercase letter followed by six digits (e.g., S001191)", 400);
    }
    ...
    if (!validPeriods.includes(period)) {
      return jsonError(`Invalid period "${period}". Must be one of: ${validPeriods.join(", ")}`, 400);
    }

    const { periodStart, periodEnd } = getPeriodDates(period);

    const cacheKey = buildCacheKey("scorecard", "calculated", `${bioguideId}:${period}`);

    const fetcher = async (): Promise<ScorecardResponse> => {
      const collectionResult: CollectionResult = await collectScorecardData(bioguideId, periodStart, periodEnd);
    ...
    if (skipCache) {
      ...
      result = await getOrFetch<ScorecardResponse>(cacheKey, fetcher, CacheTTL.SCORECARD);
    }
    ...
    if (error instanceof CongressApiError) {
      ...
      const message = error.statusCode === 404 ? "Member not found in Congress.gov" : error.message;
