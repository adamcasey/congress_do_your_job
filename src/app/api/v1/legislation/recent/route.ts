...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = buildCacheKey("congress", "recent-bills", `${limit}-${days}-${toDate.toISOString().split("T")[0]}`);
    ...
    const cached = await getOrFetch<RecentBillsResponse>(cacheKey, fetchRecentBills, CacheTTL.LEGISLATIVE_DATA);
