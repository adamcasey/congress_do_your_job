...
export async function GET(_request: NextRequest) {
  try {
    ...
    const cacheKey = buildCacheKey("congress", "stats", now.toISOString().split("T")[0]);
    ...
    const cached = await getOrFetch<CongressStatsResponse>(cacheKey, fetcher, CacheTTL.LEGISLATIVE_DATA);
