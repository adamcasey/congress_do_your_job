    ...
    <div className="w-full rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="px-6 pt-6 pb-4 border-b border-slate-100">
        <div className="flex items-start justify-between gap-4">
          <div>
            {memberName && <h2 className="text-xl font-bold text-slate-900">{memberName}</h2>}
            <p className="text-sm text-slate-500 mt-0.5">Productivity Scorecard &middot; {period}</p>
          </div>
      ...
      <div className="flex justify-center py-8 bg-slate-50/50">
        <ScorecardGauge score={scorecard.totalScore} grade={scorecard.grade} size="lg" />
      </div>
      ...
      <div className="px-6 py-4 bg-slate-50 border-t border-slate-100">
        <div className="flex items-center justify-between gap-4">
          ...
          <p className="text-xs text-slate-400 flex-shrink-0">Updated {formatDate(scorecard.calculatedAt)}</p>
        </div>
      </div>
    </div>
  );
