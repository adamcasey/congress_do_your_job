...
export async function fetchCensusData<T>(config: CensusApiConfig): Promise<T> {
  ...
  if (!response.ok) {
    ...
    logger.error("API error:", response.status, errorText, "URL:", url.toString());
  ...
  try {
    ...
    logger.error("JSON parse error:", errorMsg, "Response:", responseText.substring(0, 200), "URL:", url.toString());
...
export function isCensusArrayResponse(data: unknown): data is string[][] {
  return Array.isArray(data) && data.length >= 2 && Array.isArray(data[0]) && Array.isArray(data[1]);
}

/**
 * Extracts a value from Census API array response by column name
 */
export function extractCensusValue(data: string[][], columnName: string): string | null {
