...
const {
  useRepresentativeLookupMock,
  useAddressAutocompleteMock,
  representativeCardMock,
  districtSnapshotCardMock,
} = vi.hoisted(() => ({
  useRepresentativeLookupMock: vi.fn(),
  useAddressAutocompleteMock: vi.fn(),
  representativeCardMock: vi.fn((props: any) => (
    <div data-testid="rep-card">{props.rep.name}</div>
  )),
  districtSnapshotCardMock: vi.fn((props: any) => (
    <div data-testid="district-card">
      {props.isPlaceholder ? 'placeholder' : `${props.state}-${props.district}`}
    </div>
  )),
}))
...
describe('RepresentativeLookup', () => {
  ...
  it('submits lookups and shows empty state when no representatives are returned', async () => {
    ...
    expect(
      screen.getByText(/No representatives found for this address/i)
    ).toBeInTheDocument()
  })
