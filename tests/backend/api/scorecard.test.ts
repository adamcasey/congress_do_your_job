...
describe("Scorecard API", () => {
  ...
  it("returns 400 for invalid period parameter", async () => {
    const response = await GET(makeRequest("S001191", { period: "weekly" }), makeRouteParams("S001191"));
    const body = await response.json();
  ...
  it("accepts valid period parameters", async () => {
    for (const period of ["session", "yearly", "quarterly"]) {
      const response = await GET(makeRequest("S001191", { period }), makeRouteParams("S001191"));
      const body = await response.json();
  ...
  it("returns 404 when member not found", async () => {
    mockCollectScorecardData.mockRejectedValue(new CongressApiError("Not found", 404));
  ...
  it("returns 500 for Congress API errors", async () => {
    mockCollectScorecardData.mockRejectedValue(new CongressApiError("Server error", 500));
  ...
  it("calls collectScorecardData with correct bioguideId", async () => {
    ...
    expect(mockCollectScorecardData).toHaveBeenCalledWith("S001191", expect.any(Date), expect.any(Date));
  });
