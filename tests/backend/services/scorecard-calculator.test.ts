...
describe('calculateAttendanceScore', () => {
  it('calculates perfect attendance', () => {
    const result = calculateAttendanceScore({
      totalVotes: 50, votesParticipated: 50,
      totalHearings: 10, hearingsAttended: 10,
    })
  ...
  it('calculates partial attendance', () => {
    const result = calculateAttendanceScore({
      totalVotes: 100, votesParticipated: 80,
      totalHearings: 20, hearingsAttended: 10,
    })
  ...
  it('scores 100 when no votes or hearings scheduled', () => {
    const result = calculateAttendanceScore({
      totalVotes: 0, votesParticipated: 0,
      totalHearings: 0, hearingsAttended: 0,
    })
    expect(result.rawScore).toBe(100)
  })
  ...
  it('handles zero attendance', () => {
    const result = calculateAttendanceScore({
      totalVotes: 100, votesParticipated: 0,
      totalHearings: 20, hearingsAttended: 0,
    })
  ...
  it('includes formula in details', () => {
    const result = calculateAttendanceScore({
      totalVotes: 10, votesParticipated: 10,
      totalHearings: 5, hearingsAttended: 5,
    })
...
describe('calculateLegislationScore', () => {
  it('scores zero for no activity', () => {
    const result = calculateLegislationScore({
      billsSponsored: 0, billsCosponsored: 0,
      billsAdvancedPastCommittee: 0, billsPassedChamber: 0,
      billsEnactedIntoLaw: 0, amendmentsProposed: 0, amendmentsAdopted: 0,
    })
    expect(result.rawScore).toBe(0)
  })
  ...
  it('gives minimum floor of 10 for any activity', () => {
    const result = calculateLegislationScore({
      billsSponsored: 0, billsCosponsored: 1,
      billsAdvancedPastCommittee: 0, billsPassedChamber: 0,
      billsEnactedIntoLaw: 0, amendmentsProposed: 0, amendmentsAdopted: 0,
    })
  ...
  it('rewards enacted legislation heavily', () => {
    const withoutEnacted = calculateLegislationScore({
      billsSponsored: 2, billsCosponsored: 3,
      billsAdvancedPastCommittee: 0, billsPassedChamber: 0,
      billsEnactedIntoLaw: 0, amendmentsProposed: 1, amendmentsAdopted: 0,
    })
    const withEnacted = calculateLegislationScore({
      billsSponsored: 2, billsCosponsored: 3,
      billsAdvancedPastCommittee: 0, billsPassedChamber: 0,
      billsEnactedIntoLaw: 2, amendmentsProposed: 1, amendmentsAdopted: 0,
    })
  ...
  it('caps at 100', () => {
    const result = calculateLegislationScore({
      billsSponsored: 50, billsCosponsored: 200,
      billsAdvancedPastCommittee: 20, billsPassedChamber: 10,
      billsEnactedIntoLaw: 5, amendmentsProposed: 30, amendmentsAdopted: 15,
    })
  ...
  it('uses logarithmic scale to prevent bulk cosponsorship gaming', () => {
    const modest = calculateLegislationScore({
      billsSponsored: 0, billsCosponsored: 10,
      billsAdvancedPastCommittee: 0, billsPassedChamber: 0,
      billsEnactedIntoLaw: 0, amendmentsProposed: 0, amendmentsAdopted: 0,
    })
    const bulk = calculateLegislationScore({
      billsSponsored: 0, billsCosponsored: 100,
      billsAdvancedPastCommittee: 0, billsPassedChamber: 0,
      billsEnactedIntoLaw: 0, amendmentsProposed: 0, amendmentsAdopted: 0,
    })
...
describe('calculateBipartisanshipScore', () => {
  it('scores high for fully bipartisan record', () => {
    const result = calculateBipartisanshipScore({
      totalCosponsorships: 20, crossPartyCosponsorships: 20,
      bipartisanBillsSponsored: 5, totalBillsSponsored: 5,
    })
    expect(result.rawScore).toBe(100)
  })
  ...
  it('scores 50 (neutral) with no data', () => {
    const result = calculateBipartisanshipScore({
      totalCosponsorships: 0, crossPartyCosponsorships: 0,
      bipartisanBillsSponsored: 0, totalBillsSponsored: 0,
    })
  ...
  it('penalizes purely partisan behavior', () => {
    const result = calculateBipartisanshipScore({
      totalCosponsorships: 30, crossPartyCosponsorships: 0,
      bipartisanBillsSponsored: 0, totalBillsSponsored: 10,
    })
    expect(result.rawScore).toBe(0)
  })
})
...
describe('calculateCommitteeWorkScore', () => {
  it('rewards active committee participation', () => {
    const result = calculateCommitteeWorkScore({
      committeeMemberships: 3,
      hearingsAttended: 18, totalHearingsAvailable: 20,
      markupsParticipated: 5, totalMarkups: 6,
    })
  ...
  it('caps committee membership bonus at 10', () => {
    const manyCommittees = calculateCommitteeWorkScore({
      ...
      hearingsAttended: 10, totalHearingsAvailable: 20,
      markupsParticipated: 3, totalMarkups: 6,
    })
...
describe('calculateCivilityScore', () => {
  it('starts at baseline of 80 with no data', () => {
    const result = calculateCivilityScore({
      personalAttacksOnRecord: 0, censuresOrReprimands: 0,
      ethicsComplaintsFiled: 0, bipartisanCaucusMemberships: 0,
  ...
  it('deducts heavily for censures', () => {
    const result = calculateCivilityScore({
      personalAttacksOnRecord: 0, censuresOrReprimands: 1,
      ethicsComplaintsFiled: 0, bipartisanCaucusMemberships: 0,
      crossAisleCosponsorships: 0,
    })
  ...
  it('adds bonuses for bipartisan engagement', () => {
    const result = calculateCivilityScore({
      personalAttacksOnRecord: 0, censuresOrReprimands: 0,
      ethicsComplaintsFiled: 0, bipartisanCaucusMemberships: 3,
  ...
  it('floors at zero for extreme incivility', () => {
    const result = calculateCivilityScore({
      personalAttacksOnRecord: 5, censuresOrReprimands: 2,
      ethicsComplaintsFiled: 3, bipartisanCaucusMemberships: 0,
      crossAisleCosponsorships: 0,
    })
    expect(result.rawScore).toBe(0)
  })
...
describe('calculateTheaterRatioScore', () => {
  it('scores high for more work than theater', () => {
    const result = calculateTheaterRatioScore({
      ...
      socialMediaPostCount: 10, mediaAppearanceCount: 2,
  ...
  it('scores neutral (50) with no activity at all', () => {
    const result = calculateTheaterRatioScore({
      ...
      socialMediaPostCount: 0, mediaAppearanceCount: 0,
      pressConferencesNonLegislative: 0,
    })
    expect(result.rawScore).toBe(50)
  })
  ...
  it('punishes all-theater-no-work pattern', () => {
    const result = calculateTheaterRatioScore({
      legislativeActionsThisPeriod: 0,
      socialMediaPostCount: 100, mediaAppearanceCount: 10,
  ...
  it('considers balanced approach acceptable', () => {
    const result = calculateTheaterRatioScore({
      ...
      socialMediaPostCount: 15, mediaAppearanceCount: 3,
      pressConferencesNonLegislative: 1,
    })
...
describe('validateWeights', () => {
  ...
  it('rejects weights that do not sum to 1.0', () => {
    const badWeights: ScoringWeights = {
      ...DEFAULT_WEIGHTS,
      [ScoreCategory.ATTENDANCE]: 0.50, // pushes sum over 1.0
    }
...
describe('calculateScorecard', () => {
  ...
  it('total score is sum of weighted category scores', () => {
    ...
    const manualSum = scorecard.categoryScores.reduce(
  ...
  it('accepts custom weights', () => {
    ...
    const customWeights: ScoringWeights = {
      [ScoreCategory.ATTENDANCE]: 0.30,
      [ScoreCategory.LEGISLATION]: 0.30,
      [ScoreCategory.BIPARTISANSHIP]: 0.10,
      [ScoreCategory.COMMITTEE_WORK]: 0.10,
      [ScoreCategory.CIVILITY]: 0.10,
      [ScoreCategory.THEATER_RATIO]: 0.10,
    }
  ...
  it('produces different scores for different members', () => {
    const hardWorker = makeScoringInput({
      ...
      theaterRatio: { legislativeActionsThisPeriod: 25, socialMediaPostCount: 5, mediaAppearanceCount: 1, pressConferencesNonLegislative: 0 },
    })
    const showboat = makeScoringInput({
      attendance: { totalVotes: 100, votesParticipated: 40, totalHearings: 20, hearingsAttended: 5 },
      theaterRatio: { legislativeActionsThisPeriod: 2, socialMediaPostCount: 200, mediaAppearanceCount: 15, pressConferencesNonLegislative: 8 },
    })
