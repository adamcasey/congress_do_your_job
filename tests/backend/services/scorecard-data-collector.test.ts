...
function setupMocks(member: Member = makeMember(), sponsoredBills: Bill[] = [], cosponsoredBills: Bill[] = []) {
...
describe("legislation data collection", () => {
  ...
  it("handles bills with no latestAction gracefully", async () => {
    const sponsored = [makeBill({ number: "1", updateDate: "2025-03-01", latestAction: undefined })];
...
describe("error handling", () => {
  it("propagates Congress.gov API errors", async () => {
    ...
    await expect(collectScorecardData("INVALID", PERIOD_START, PERIOD_END, { skipCache: true })).rejects.toThrow(
      "Member not found",
    );
  });
});
